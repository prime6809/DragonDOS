...
;
; Disassembled 2004-10-28, P.Harvey-Smith.
;
; Ported to the Dragon Alpha/Professional hardware,
; 2004-10-29 P.Harvey-Smith.
;
; Problem / Fault fixes from P.G.Scott's May 1985 Dragon User article may be 
; enabled by defining PGSFix at compile time.
;
; The default Makefile builds both versions of the ROM with the fixes applied
; or not.
;
; ddos10.rom	is the vanilla Dragon DOS as supplied on eprom, and should be 
;		binary identical to it.
;
; ddos12.rom	is Dragon DOS with P.G.Scot's fixes compiled in.
;
; It is my eventual intention to also merge in the changes from Dragon DOS 2, 
; as supplied with the Dragon Alpha / Professional.
;
...
DC000   FCC     /DK/		; Disk cartrage auto-init identifier.
...
; points to be called with jsr [entry]
	...
	FDB	CmdRenameMC		; Rename a file 
	else
        FDB     CmdProtect		; Protect/unprotect file
        FDB     CmdRename		; Rename a file 
        endc
...
LC054   LDB     #$20			; Set hardware control mask
        STB     DosHWMaskFF48
...
LC18C   LDX     #PIA0CRA		; Point to PIA CR A		
        LDB     #$34
        LDU     #SetPIA
        EXG     U,PC			; Go Do it dunno why not jsr..rts :)

LC196   LDU     #SetPIA
        EXG     U,PC			; Go Do it dunno why not jsr..rts :)

LC19B   NOP
        LDB     #$37			; enable FIRQ from FDC
        STB     PIACRB
...
LC1B8   LDY     <DBZero
        ...
        BCC     LC1CF			; No error : check verify		
        BRA     LC1FD
...
LC1E4   TST     DosErrorMask		; is this write sector ?
        ...
        BPL     LC1FD			; no, skip ahead
	
LC1FA   LBSR    DosDoReadSec2		; verify the write
LC1FD   LEAS    1,S
        ROL     4,S
...
LC21F   STA     ,X			; update track table
        LDU     #ResetPIA		; address to call
        EXG     U,PC			; call without using stack	

LC226   LDU     #ResetPIA2		; address to call
        EXG     U,PC			; call without using stack
...
LC27F   ORA     #NMIEn+MotorOn		; Mask in nmi enable & Motor on bits
        PSHS    A
...
LC2F2   SYNC
LC2F3   LDA     DPDataReg		; read byte from WD
        LDB     DPPIADB			; clear interrupt
...
LC394 
	ifdef	DragonAlpha
	LBRA	AlphaDskCtl		; Write control reg
	ELSE
	STA     DPDskCtl		; Write control reg
        RTS
	ENDC
...
IRQSrv  
	ifdef	PGSFix
	BRA	LC72D			; branch to fix code
	ELSE
        TST     <DosIOInProgress 	; Doing IO ?
	ENDC
	
IRQSrv2 BNE     LC71E			; Yes: don't time out
...
LC721   JMP     >DosHookSysError
	...
	TFR	A,DP
	TST	<DosIOInProgress 	; Doing IO ?
	BRA	IRQSrv2			; Branch back to IRQ service

LC72D	PULS	B			; restore B
	JMP     >BasIRQVec		; Jump to BASIC IRQ
	NOP				; resync locations in output
	NOP
	else
;
...
LC724   TFR     S,D			; Get stack pointer in D
...
; This code is used if we are writing	
        ...
        TFR     X,D			; Deleted from DDv2 as redundent
        DECB				; Deleted from DDv2 as redundent
        BCC     LCA44			; Deleted from DDv2 as redundent
...
LCFEE   DEC     3,S			; decrement entry count
        ...
        PULS    X			; restore FCB pointer
        BNE     LCFB5			; branch if errors in DOSGetDirEntry2
...
LD08B   LDX     #BAMLSNPerSec		; get BAM entries / sec
        LEAS    4,S			; clean up stack	
        LDB     ,S+			; get count
        BNE     GoDeleteExtent		; loop again if not zero
        RTS
...
;
	...
	CMPB    #ErrNE		; File not exist ?
        BEQ     LD36E			; does not exit, create it
...
LD3B2   LDA     CopySrcFCBNo,S		; retrieve source FCB no
        ...
        LDY     CopyIOBufPtr,S
        LDX     BasVarEnd
        JSR     >DOSFRead		; go read source file
        BNE     LD36B			; error : exit
        ...
        LDU     CopyIOBufPtr,S
        LDX     BasVarEnd
        JSR     >DOSFWrite		; go write to destination
	
	BNE     LD36B			; error exit
        BRA     LD375

LD3DB   JSR     >DOSCloseAll		; close all files
        LBNE    LD36B			; error exit
...
CmdMerge   
	JSR     >DosValidateAndOpenBAS	; open basic program file
        BNE     LD433			; error, exit
	
        JSR     >ReadHeadGetTypeA	; read header, get filetype 		
        BNE     LD433			; error, exit
	...
	PSHS    Y,U			; save regs
        ...
        JSR     >DoBasLoad		; glo load program
...
GenFMError   
	LDB     #BErrFM			; FM error
LD433   JMP     >DosHookSysError	; call error hook
...
LD4A2   LEAS    2,S			; drop return address
        LDB     #$01			; flag run
...
CmdLoad	CLRB				; flag load
        STB     DosRunLoadFlag		
        JSR     >DosValidateAndOpenBAS	; check and open the basic program file
        BNE     LD433			; error, exit
	
        JSR     >ReadHeadGetTypeA	; read header of basic program
        LBNE    LD433			; error, exit
	
        CMPA    #FTypeBas		; is the file of type basic?
...
LD4ED   LDA     <DosCurrCtrlBlk		; get current control block
        ...
        JSR     >DOSFRead		; go read it
	
        LBNE    DosHookSysError		; error, try catching it
...
LD524   LDY     HdrLen,X		; get file length into Y
        ...
        JSR     >DOSFRead		; go read the file into memory
	
        LBNE    DosHookSysError		; try catching error if any
	
        TST     DosRunLoadFlag		; test if we should run loaded code?
        BEQ     LD503			; nope return to caller
...
LD572   JSR     >DosOpenFileExtY	; get the supplied filename & try opening
        BEQ     LD57B			; all ok, skip on and create
	
        CMPB    #ErrNE		; File does not exist error?
        BNE     LD5BB			; no, skip to error handler
	
LD57B   JSR     >DOSCreateFile		; (re)create file
        BNE     LD5BB			; error skip to handler
        RTS
;
...
LD5A1   LDX     #DosCurFilename		; point to DosCurFilename, used as header buffer
        ...
        BEQ     LD5BE			; if it wrote ok, go write rest of file

LD5BB   JMP     >DosHookSysError	; call error handler
...
LD5BE   LDA     <DosCurrCtrlBlk		; get current control block
        ...
        LDY     <DBZero	; Y=0
        JSR     >DOSFWrite		; write the rest of the file
        BNE     LD5BB			; branch on error
...
CmdChain   
	JSR     >DosValidateAndOpenBAS	; check and open the basic program file
        LBNE    LD6F8			; error : exit
	
        JSR     >ReadHeadGetTypeA	; go read the file's header, type in A
        LBNE    DosHookSysError		; error : exit
	
        CMPA    #FTypeBas		; make sure it's a basic program type
        LBNE    GenFMError		; nope : generate FM error
        
	JSR     >LD669			; check and move strings to string area if needed
	
        LDD     HdrLen,X		; get length of file to chain
        STD     DosCurFilename+6	; save it 
...
LD61F   LDD     ,S			; get offset 
	...
	LDU     BasVarEnd		; get address of end of vars
        LDY     <BasVarArrayAddr	; get address of arrays
        JSR     <BasChrGetCurr		; get next character from basic
        BEQ     LD652			; no character, skip on
	
        LDB     #','			; check for a comma
        JSR     >VarCKChar		; error if not
	
        JSR     >BasGetLineNo		; get supplied line number, convert to binary 
        JSR     >BasVect1a		; "erase" basic vars
        STU     BasVarEnd		; restore end of variables
	
        STY     <BasVarArrayAddr	; restore array address
        LDY     DosCurFilename+6	; get length of program to chain
        JSR     >LD4ED			; go load it
	
        LDD     <BasTempLine		; get line to start running at
        JSR     >BasSkipLineNo		; go skip to it
        BRA     LD661

LD652   JSR     >BasVect1		; "erase" basic vars
        STU     BasVarEnd		; restore end of variables
...
LD669   LDU     <BasAddrSigByte		; get address of current basic program pointer
        ...
        LDX     <BasVarSimpleAddr	; point at simple variables
        LEAX    2,X			; add 2
...
LD698   JSR     >LD6AD			; check string descriptot and move to string space if needed
...
LD6A5   JSR     >VarGarbageCollect	; garbage collect
        PULS    X,U			; restore regs
        STU     <BasAddrSigByte		; restore basic pointer
        RTS
...
CmdCreate   
	JSR     >DosValidateAndOpenDAT	; validate and open the supplied filename
...
LD732   LDA     <DosCurrCtrlBlk		; get current control block
        JSR     >DOSCreateFile		; go create the file
...
LD76E   JSR     >ExtendFile		; extend file
...
CmdKill   
	JSR     >DosGetFilenameAndOpen	; find the filename and open it
        BNE     LD77E			; error, exit
	
        JSR     >DOSDeleteFile		; delete the file
...
LD792   JSR     DOSProtectMC		; go turn it off/on
        BNE     LD77E			; Go handle errors
        RTS

LD798   JSR     <BasChrGet		; get a character from basic
        JSR     DosGetFilenameAndOpen	; get the filename and open file
...
CmdRename   
	JSR     >DosGetFilenameAndOpen	; get first filename and open it
        ...
        JSR     >DosGetFilenameAndOpen	; get second filename and open
        ...
        JSR     >DOSRename		; go change the name.
...
CmdFLRead   
	BSR     FLReadSetup		; get setup for read open file etc
        LDA     #$FF			; flag FLREAD
        STA     DosFlFreadFlag
        JSR     >LD969			; do the read
        BSR     LD803			; fix the pointers
        JMP     >BasLineInputEntry	; finish off input put it into string
	

FLReadSetup   
	JSR     >DosValidateAndOpenDAT	; validate filename and open it
        BNE     LD82D			; error, exit
	
        JSR     >FLRWGetNamePos		; get name of file, and filepointer pos
        JSR     >DosFCBNoToAddr		; get address of FCB in X
...
LD7F0   JSR     >LD9DC			; check and read file
        LDX     #BasLinInpBuff		; point at basic input buffer
        CLR     ,X			; clear fist byte
        JSR     <BasChrGet		; go read from buffer.
        RTS				
...
;
LD803   JSR     >DosFCBNoToAddr		; get address of FCB
...
LD849   JSR     <BasChrGetCurr		; get current character from basic
        ...
        BEQ     LD82C			; yep get data to read/write
...
LD86A   CMPA    #DTokFOR		; for token?
        ...
        JSR     >Get8BitorError		; go get an 8 bit record size
...
LD8A2   JMP     >DosHookSysError	; call error handler
...
CmdFWrite   
	JSR     >DosValidateAndOpenDAT	; open and validate file
        BEQ     LD8B3			; no error, branch on
	
        CMPB    #ErrNE			; Does the file not exist?
        BNE     LD8A2			; no it was another, error handle it.
	
        JSR     >DOSCreateFile		; create the file if it does not exist
        BNE     LD8A2			; error handle it
	
LD8B3   JSR     >FLRWGetNamePos		; get name and filepointer to start writing at
        JSR     >FindFreeDiskBuffer	; find a free disk buffer, to write data into
...
LD8D8   TST     <DosBytesInDTA		; any bytes in buffer?
        ...
        JSR     >DOSFWrite		; go write to the file
...
LD914   JMP     >BasDNError		; device number error
...
LDBFE   LDB     #ErrBT			; flag boot error
...
LDC0D   LDA     #BootFirstSector 	; Read boot sector	
        STA     <DskSectorNo
        STB     <DosDriveNo		; Set drive number
...
LDC2A   JSR     >DosDoReadSec		; Read a sector
	...
	JMP     >BootEntryAddr		; jump to loaded boot cod
...
LDD0A   JSR     DosDoWriteSecV		; go write it
...
DosHookEOF   
	TST     <BasVarType		; check var type
        ...
        LDY     #DosExtDat
        JSR     DosOpenFileExtY		; validate and open file
...
; 
FuncLoc
	JSR     DosValidateAndOpenDAT	; Validate and open file
        BNE     JMPSysErrHook			; error : exit
...
RamHookTable   
	...
	ifdef	PGSFix
	FDB	DosHookReadInputFix	; read line of input (fixed)
	else
...
DosErrorCodeTable
	...
	ifdef	DragonAlpha
DosSignonMess	FCC		/ALPHA-DOS/
	ELSE
DosSignonMess	FCC		/DRAGONDOS/
	ENDC
	...
	ifdef	PGSFix
LDFCA	LDA	DosTempFCBNo		; recover fileno
	TST	[DosSaveFCB]		; test first character of filename
	rts
	else
...
; Called with EXG as that way we can return and still leave things on the stack.
; Returns saved PIA registers on the stack

SetPIA  LDA     ,X			; get control register side A
        PSHS    A			; save on stack
        STB     ,X++			; set control register side A, advance pointer
        LDA     ,X			; get control register side B
        PSHS    A			; save it in stack
        STB     ,X			; set control register side B
        LDX     #PIA1CRA		; point to PIA 1
        TFR     U,PC			; return

; Reset PIAs from registers saved on the satack, uses same EXG trick as above

ResetPIA   
	LDX     #PIA1CRB+2		; point to 2 after control reg, as we use pre-decrement below
ResetPIA2   
	PULS    A			; get saved control register B value
        STA     ,--X			; decrement pointer then write it
        PULS    A			; get saved control register A value
        STA     ,--X			; decrement pointer then write it
        LDX     #PIA0CRB+2		; point to 2 after PIA0 CRB
        TFR     U,PC			; return
...
ADriveTab
	...
	ifdef	PGSFix
LDFF3	TSTB				; check for zero length filename
	BNE	LDFF9			; no, save it and return
	LDB	#ErrFS			; else generate FS error
	RTS
LDFF9
	LDA	DosDefDriveNo		; get dos default drive number
	JMP	LC7C1			; return to patched code
	endc		
